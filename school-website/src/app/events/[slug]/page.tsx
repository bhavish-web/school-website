    ...
    <>
      <section className="relative bg-[var(--color-ink)]">
        ...
        <Container className="relative -mt-24 pb-16">
          <Reveal>
            ...
            <h1 className="mt-3 max-w-2xl font-display font-medium text-[2rem] leading-tight text-white md:text-[2.8rem]">
              {event.title}
            </h1>
          </Reveal>
        </Container>
      ...
      {event.gallery.length > 0 && (
        <section className="bg-[var(--color-paper)] py-16">
          <Container>
            <Reveal>
              <h2 className="font-display font-medium text-[1.5rem] text-[var(--color-ink)]">Gallery</h2>
            </Reveal>
      ...
      {related.length > 0 && (
        <section className="bg-white py-16">
          <Container>
            <Reveal>
              <h2 className="font-display font-medium text-[1.5rem] text-[var(--color-ink)]">Related Events</h2>
            </Reveal>
              ...
              {related.map((e) => (
                <RevealItem key={e.slug}>
                  <a href={`/events/${e.slug}`} className="group block">
                    ...
                    <h3 className="mt-3 font-display font-medium text-[1.05rem] text-[var(--color-ink)] transition-colors group-hover:text-[var(--color-brass)]">
                      {e.title}
                    </h3>
                  </a>
                </RevealItem>
