  ...
  return (
    <>
      <section className="grain-overlay bg-[var(--color-ink)] py-24 md:py-32">
        <Container>
          <Reveal>
            <p className="eyebrow text-[var(--color-gold-bright)]">Campus</p>
      ...
      {Object.entries(grouped).map(([category, items], i) => (
        <section key={category} className={i % 2 === 0 ? "bg-white" : "bg-[var(--color-paper)]"}>
          <Container className="py-20">
            <Reveal>
              <h2 className="font-display font-medium text-[1.7rem] text-[var(--color-ink)]">{category}</h2>
            </Reveal>
            <RevealGroup className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {items.map((f) => (
                <RevealItem key={f.id}>
                  ...
                  <h3 className="mt-4 font-display font-medium text-[1.1rem] text-[var(--color-ink)]">{f.name}</h3>
