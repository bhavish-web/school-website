    ...
    >
      <Container className="flex h-20 items-center justify-between">
        <Link href="/" className="group flex items-center gap-3" aria-label={`${school.name} — home`}>
          ...
          <span className="hidden font-display font-medium text-[1.05rem] font-medium leading-tight text-[var(--color-ink)] sm:block">
            {school.name}
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-9 lg:flex" aria-label="Primary">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="group relative text-[0.88rem] font-medium tracking-[0.01em] text-[var(--color-ink)]/70 transition-colors hover:text-[var(--color-ink)]"
            >
              {l.label}
              <span className="absolute -bottom-1.5 left-0 h-[1.5px] w-0 rounded-full bg-[var(--color-brass)] transition-all duration-300 group-hover:w-full" />
            </Link>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <Container className="flex h-20 items-center justify-between">
              <span className="font-display font-medium text-[1.1rem]">{school.name}</span>
              <button
              ...
              {links.map((l, i) => (
                ...
                >
                  <Link
                    href={l.href}
                    onClick={() => setOpen(false)}
                    className="block border-b border-white/10 py-4 font-display font-medium text-[1.6rem] text-white/90 transition-colors hover:text-[var(--color-gold-bright)]"
                  >
                    {l.label}
                  </Link>
                </motion.div>
