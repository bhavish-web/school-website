    ...
    <section className="bg-[var(--color-paper)] py-24 md:py-32">
      <Container>
        ...
        <div className="mt-16 grid grid-cols-1 gap-4 md:grid-cols-6 md:grid-rows-2">
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl md:col-span-4 md:row-span-2 md:aspect-auto">
            <Media src={first.image.src} alt={first.image.alt} />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-6">
              <p className="font-display text-[1.4rem] text-white">{first.name}</p>
            </div>
          </div>
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl md:col-span-2">
            <Media src={second.image.src} alt={second.image.alt} />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/55 to-transparent p-5">
              <p className="font-display text-[1.1rem] text-white">{second.name}</p>
            </div>
          </div>
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl md:col-span-2">
            <Media src={third.image.src} alt={third.image.alt} />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/55 to-transparent p-5">
              <p className="font-display text-[1.1rem] text-white">{third.name}</p>
            </div>
          </div>
        </div>
