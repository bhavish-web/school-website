...
export function QuickActions() {
  return (
    <div className="relative z-10 -mt-10 md:-mt-12">
      <Container>
        <div className="flex gap-3 overflow-x-auto rounded-2xl bg-white p-3 shadow-[0_18px_50px_-20px_rgba(20,33,61,0.35)] md:grid md:grid-cols-6 md:gap-2 md:overflow-visible">
          {actions.map((a) => {
            const Icon = a.icon;
            return (
              <Link
                key={a.label}
                href={a.href}
                className="flex min-w-[130px] flex-col items-center gap-2 rounded-xl px-3 py-4 text-center transition-colors hover:bg-[var(--color-paper)] md:min-w-0"
              >
                <Icon size={20} strokeWidth={1.5} className="text-[var(--color-brass)]" />
