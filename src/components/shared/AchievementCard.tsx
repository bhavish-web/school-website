...
export function AchievementCard({ achievement: a }: { achievement: Achievement }) {
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-[var(--color-line)]">
      <div className="relative aspect-[16/10] w-full">
        <Media src={a.image.src} alt={a.image.alt} />
      </div>
      <div className="flex flex-1 flex-col gap-2 p-6">
        <span className="text-[0.78rem] font-medium text-[var(--color-brass)]">{a.category}</span>
        ...
        <div className="mt-auto flex items-center justify-between pt-4 text-[0.82rem] text-[var(--color-slate)]">
          <span>{a.date}</span>
          <span className="font-medium text-[var(--color-ink)]">{a.result}</span>
        </div>
      </div>
    </article>
  );
